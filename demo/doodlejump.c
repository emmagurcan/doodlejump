...
const size_t SPRITE_RESOLUTION = 30;
...
//death screen 
const image_t DEATH_SCREEN_IMAGE = {"media/death_screen.png", 500, 1000};
...
const double MONSTER_SCORE = 20.0;
...
const image_t SHOT_IMAGE = {"media/flower_bullet.png", 20, 20};
...
const double LEVEL_TRIGGERS[4] = {2000, 5000, 10000, 15000};
const level_info_t LEVEL_INFO[4] = {
    {10.0, 100.0, 5, 0, 100.0, 150, 0, 0, 0, 0.5},
    {50.0, 150.0, 10, 0, 100, 150, 0, 0.2, 0, 0.3},
    {50.0, 200.0, 15, 2, 300.0, 500.0, 0, 0.2, 0.1, 0.2},
    {150.0, 280.0, 20, 4, 80.0, 300.0, 0.5, 0.6, 0.3, 0.1}
};
...
double make_constant_platforms(scene_t *scene, double start_y, double height, double min_jump, double max_jump, double moving_plat_ratio, double spring_ratio, double jet_ratio) {
    double y_pos = start_y;
    while (y_pos < start_y + height) {
        y_pos += rand_range(min_jump, max_jump);
        ...
        if (velo.x == 0.0 && rand_range(0, 1) < spring_ratio) {
        ...
        else if (velo.x == 0.0 && rand_range(0, 1) < jet_ratio) {
            ...
            make_jet(scene, base, JET_SIZE, JET_MASS, JET_COLOR, ACC, LOW_ACC, HIGH_ACC, STOP_ACC, START_DEC, JET_IMAGE);
        }
...
// Spawns in monsters
double make_monsters(scene_t *scene, double start_y, double height, double num_monsters_per_screen, double min_distance, double max_distance, double moving_monster_ratio) {
    double num_monsters = num_monsters_per_screen * height / WINDOW_MAX.y;

    if (rand_range(0.0, 1.0) < num_monsters - (double) floor(num_monsters)) {
        num_monsters ++;
    }
    num_monsters = (size_t) num_monsters;

    double y_pos = start_y;
    size_t curr_num = 0;
    while (curr_num < num_monsters && y_pos < start_y + height) {
        y_pos += rand_range(min_distance, max_distance);
        vector_t center = {rand_range(WINDOW_MIN.x, WINDOW_MAX.x - EPSILON), y_pos};
        vector_t velo = (vector_t) {0.0, 0.0};
        rgb_color_t color = MONSTER_COLOR;
        if (rand_range(0, 1) < moving_monster_ratio) {
            if (rand_range(0, 1) > 0.5){
        ...
        if (y_pos < start_y + height) {
            make_monster(scene, center, MONSTER_RAD, MONSTER_MASS, velo, color, MONSTER_SCORE, ACC, BOUNCE_HEIGHT, MONSTER_RESOLUTION, MONSTER_IMAGE);
            curr_num ++;
        }
    }
...
// Adds one more screen height section to the scene. Returns the elevation of the highest platform created.
double extend_scene(scene_t *scene, double start_y, level_info_t level_info) {
    double max_y = make_constant_platforms(scene, start_y, WINDOW_MAX.y - WINDOW_MIN.y, level_info.min_jump, level_info.max_jump, level_info.moving_plat_ratio, level_info.spring_ratio, level_info.jet_ratio);
    make_breaking_platforms(scene, start_y, WINDOW_MAX.y - WINDOW_MIN.y, level_info.num_breaking);
    make_monsters(scene, start_y, WINDOW_MAX.y - WINDOW_MIN.y, level_info.num_monsters, level_info.min_monster_distance, level_info.max_monster_distance, level_info.moving_monster_ratio);
...
double *get_sprite_score(scene_t *scene) {
    body_t *sprite = scene_get_body(scene, 1);
    return (double *)body_get_info(sprite);
}

void increase_score(scene_t *scene, double amount) {
    double *score = get_sprite_score(scene);
    *score += amount;
}
...
double move_screen(scene_t *scene, double highest_plat_elevation, size_t level_index) {
    ...
    if (y_change > 0) {
        ...
        while (highest_plat_elevation < elevation + (WINDOW_MAX.y - WINDOW_MIN.y) + y_change) {
            highest_plat_elevation = extend_scene(scene, highest_plat_elevation - elevation, level_info) + elevation;
        }
...
void defend(scene_t *scene) {
    ...
    body_t *shot = make_shot(scene, location, SHOT_SIZE, SHOT_MASS, DEFEND_COLOR, DEFEND_SHOT_VELOCITY, SHOT_RESOLUTION, SHOT_IMAGE);
...
void start_screen(scene_t *scene) {
    ...
    body_set_type(sprite, SPRITE_START);
...
void death_screen(scene_t *scene){
    // scene = reset(scene); 
   ...
   // counter sprite movement: 
    ...
    body_set_velocity(sprite, VEC_ZERO);
    list_t *bodies = list_init(1, NULL);
    list_add(bodies, sprite);
    double *acc = malloc(sizeof(double));
    *acc = -ACC;
...
scene_t *death(scene_t *scene) {
    ...
    scene = reset(scene);
    if (body_get_type(sprite) == SPRITE_DEAD){
        death_screen(scene);
    }
    return scene;
}
...
void on_key(scene_t *scene, char key, key_event_type_t type, double held_time) {
    ...
    if (type == KEY_PRESSED) {
        switch (key) {
            ...
            case SDLK_s: 
                if (body_get_type(sprite) == SPRITE_DEAD || body_get_type(sprite) == SPRITE_START){
                    body_set_type(sprite, SPRITE_RESET);
                }
                else{
                    body_set_type(sprite, SPRITE_DEAD);
                }
...
int main(int argc, char *argv[]) {
    ...
    body_type_t body_type = body_get_type(sprite);
    start_screen(scene);

    soundeffx_init();
    soundeffx_play_background_music();
    ...
    while (!sdl_is_done(scene)) {
        ...
        double dt = time_since_last_tick();
            ...
            if (body_get_centroid(sprite).y < WINDOW_MIN.y){
                body_set_type(scene_get_body(scene, 1), SPRITE_DEAD);
            }

            bool paused = (body_get_type(scene_get_body(scene, 1)) == body_type);
            if (!paused){           
                if ((body_get_type(scene_get_body(scene, 1)) == SPRITE_DEAD || body_get_type(scene_get_body(scene, 1)) == SPRITE_RESET)) {              
                    scene = death(scene);
                    if (body_get_type(scene_get_body(scene, 1)) == SPRITE){
                        vector_t first_plat_centroid = {body_get_centroid(sprite).x, rand_range(PLATFORM_HEIGHT, body_get_centroid(sprite).y * 0.5)};
                        make_platform(scene, first_plat_centroid, PLATFORM_WIDTH, PLATFORM_HEIGHT, PLATFORM_MASS, VEC_ZERO, PLATFORM_COLOR, PLATFORM_IMAGE, ACC, BOUNCE_HEIGHT, platform_collision);
        ...
        body_type = body_get_type(scene_get_body(scene, 1));
