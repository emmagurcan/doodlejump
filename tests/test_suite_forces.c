...
void test_destructive_collisions() {
    const double DT = 0.1;
    const double V = 1.23;
...
void test_physics_collisions() {
    ...
    vector_t initial_separation =
        {SEPARATION_AT_COLLISION + V * DT * (TICKS_TO_COLLISION - 0.5), 0};
    body_set_centroid(body1, initial_separation);
    body_set_velocity(body1, (vector_t) {-V, 0});
    scene_add_body(scene, body1);
    ...
    create_physics_collision(scene, E, body2, body1);

    for (int i = 0; i < TICKS_TO_COLLISION * 2; i++) {
        scene_tick(scene, DT);
        // Before collision, body1 travels left and body 2 is still, after is vice versa.
        if (i < TICKS_TO_COLLISION) {
            assert(vec_isclose(body_get_velocity(body1), (vector_t) {-V, 0}));
        }
        else {
            assert(vec_isclose(body_get_velocity(body1), (vector_t) {0, 0}));
            assert(vec_isclose(body_get_velocity(body2), (vector_t) {-V, 0.0}));
            assert(vec_isclose(body_get_velocity(body1), (vector_t) {0, 0}));
        }
    }
    scene_free(scene);
...
int main(int argc, char *argv[]) {
    ...
    DO_TEST(test_destructive_collisions)
    DO_TEST(test_physics_collisions)
