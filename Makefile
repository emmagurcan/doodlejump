...
DEMOS = bounce gravity pacman nbodies damping spaceinvaders pegs breakout doodlejump
...
STUDENT_LIBS = vector list shape polygon skin body scene rand_utils forces collision game_make_objects soundeffx
...
CFLAGS = -Iinclude $(shell sdl2-config --cflags | sed -e "s/include\/SDL2/include/") -Wall -g -fno-omit-frame-pointer -fsanitize=address -Wno-nullability-completeness -arch arm64
...
all: $(DEMO_BINS) # only compiles demos
...
# "To build 'all', make sure all files in BINS are up to date."
# You can execute this rule by running the command "make all", or just "make".
all: $(DEMO_BINS) # now it only compiles the demo bins
...
bin/doodlejump.exe: out/doodlejump.obj out/sdl_wrapper.obj $(STUDENT_OBJS)
	$(CC) $^ $(CFLAGS) -link $(LINKEROPTS) $(LIBS) -out:"$@"
